...
xlabel('time(sec)')
ylabel('Error Covariance Amliptude')
title('M(t) elements')
grid on
%noise
...
for i = 1:length(t)-1
x1(i+1)=x1(i)+dt*(-3*x1(i)-x3(i)+1+w(i,1));
x2(i+1)=x2(i)+dt*(-2*x2(i)+x3(i)+w(i,2));
x3(i+1)=x3(i)+dt*(x2(i)-9*x3(i)+5+w(i,1));
...
for i = 1:length(t)-1
x1_hat(i+1)=x1_hat(i)+dt*(-3*x1_hat(i)-x3_hat(i)+1+L(1,:,i)*(y(i)-x1_hat(i)-2*x2_hat(i)));
x2_hat(i+1)=x2_hat(i)+dt*(-2*x2_hat(i)+x3_hat(i)+L(1,:,i)*(y(i)-x1_hat(i)-2*x2_hat(i)));
x3_hat(i+1)=x3_hat(i)+dt*(x2_hat(i)-9*x3_hat(i)+5+L(1,:,i)*(y(i)-x1_hat(i)-2*x2_hat(i)));
end
%State_variable_plot
figure
plot (t,x1)
